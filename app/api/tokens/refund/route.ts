...
import { liveRefundTokens } from '@/lib/live-token-ledger';
...
export async function POST(request: Request) {
  ...
  const gate = assertLivePaidSaaSAllowed('token_refund');
  ...
  const transaction = {
    ...
    idempotencyKey: String(body.idempotencyKey ?? `refund_${Date.now()}`),
    createdAt: new Date().toISOString()
  };
  const result = await liveRefundTokens(transaction, String(body.reason ?? 'job_failed_refund'));
